...
export default function RegisterPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!form.street.trim()) { setError("丁目・番地・号を入力してください。"); return; }
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    ...
                    <input name="postalCode" value={form.postalCode} onChange={onChange}
                        inputMode="numeric" placeholder="123-4567"
                        className="w-full p-2 border rounded mb-3" />

                    {/* 都道府県（選單） */}
                    <label className="block text-sm mb-1">都道府県</label>
                    <select name="prefecture" value={form.prefecture} onChange={onChange}
                        className="w-full p-2 border rounded mb-3">
                        <option value="">選択してください</option>
                    ...
                    {/* 丁目・番地・号（数字は半角数字） */}
                    <label className="block text-sm mb-1">丁目・番地・号（数字は半角）</label>
                    ...
                    <input name="room" value={form.room} onChange={onChange}
